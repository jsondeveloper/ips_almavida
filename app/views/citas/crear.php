...
<head>
<meta charset="UTF-8">
<title>Nueva Cita - IPS Alma Vida</title>
...
<h3 class="mb-4"><i class="bi bi-calendar-event"></i> Registrar Cita</h3>
...
<div class="col-md-6">
    ...
    <select name="paciente" id="paciente" class="form-control" required>
        ...
        <option value="<?= $pac['id'] ?>" data-empresa="<?= htmlspecialchars($pac['empresa'] ?? '') ?>">
            <?= $pac['nombre_completo'] ?>
        </option>
        <?php endforeach; ?>
    </select>
...
<div class="col-md-6">
    ...
    <input name="empresa" id="empresa" class="form-control" readonly required>
    ...
    .then(ocupadas => {
        ...
        for(let h=8; h<=17; h++){
            ...
            horaSelect.innerHTML += `
                <option value="${fecha}T${hora24}" ${ocupada ? 'disabled' : ''} class="${ocupada ? 'hora-ocupada' : 'hora-disponible'}">
